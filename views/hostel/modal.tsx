...
const HostelBooking: React.FC<HostelBookingProps> = ({ isOpen, onClose }) => {
    const t = useTranslations("HomePage");
  const { id } = useParams(); 
  const [hostel, setHostel] = useState<{ name: string; photo: string } | null>(null);
  ...
  useEffect(() => { 
    if (id) {
      fetch(`https://shinely.tanuweb.cloud/api/v1/hostel/${id}`)
        .then((res) => res.json())
        .then((data) => {
          setHostel({ name: data.name, photo: data.photo });
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const bookingData = { ...formData, hostel: id };

    try {
      const response = await fetch("https://shinely.tanuweb.cloud/api/v1/hostelbooking/", {
        method: "POST",
