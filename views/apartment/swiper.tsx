...
const ApartmentSwiper = ({ apartment, showArrow }: Props) => {
  const t = useTranslations("HomePage");
  const router = useRouter();

  const handleSlideClick = (id: string) => {
    router.push(`/accommodation/apartments/${id}`);
  };
  ...
  return (
    <div className="w-full my-8 lg:my-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex items-center justify-center gap-4 max-w-[1200px] w-full py-0 md:px-0 flex-wrap mb-4">
        <span className="text-4xl font-bold text-[#5C98F2]">{t("apartment")}</span>
      </div>
      ...
      >
        {apartment.map((apartmentItem) => (
          <SwiperSlide key={apartmentItem._id} className="swiper-slide">
            <div
              onClick={() => handleSlideClick(apartmentItem._id)}
              ...
              style={{
                backgroundImage: `url(https://shinely.tanuweb.cloud/uploads/${apartmentItem.photo})`,
              }}
            >
              {/* apartment Name */}
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white transition-opacity duration-500 ">
                <h3 className="text-2xl sm:text-3xl font-bold text-center  group-hover:opacity-0">
                  {apartmentItem.name}
                </h3>
              </div>
              ...
              <div className="absolute inset-0 flex flex-col justify-center items-center text-white opacity-0 transition-opacity duration-1000 group-hover:opacity-100 px-4 text-center">
                <p className="mt-2 text-sm sm:text-base text-justify line-clamp-6">{apartmentItem.description}</p>
              </div>
            </div>
          </SwiperSlide>
        ))}
